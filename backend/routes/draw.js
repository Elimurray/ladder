...
router.post(
  ...
  async (req, res) => {
    ...
    `);
      ...
      const fiveThirtyPairings = []; // Juniors OR adults with BOTH wanting 5:00pm
      const juniorPairings = []; // Juniors who DON'T both want 5:00pm
      const regularPairings = [];

      for (const pairing of pairings) {
        const hasJunior = pairing.player1_junior || pairing.player2_junior;

        // BOTH players must want 5:00pm to be eligible for that slot
        const bothWant530 =
          pairing.player1_earliest === "5:00pm" &&
          pairing.player2_earliest === "5:00pm";

        if (bothWant530) {
          // Anyone (junior or adult) who both want 5:00pm goes here
          fiveThirtyPairings.push(pairing);
        } else if (hasJunior) {
          // Juniors who don't both want 5:00pm
      ...
      console.log(
        "5:00pm preference pairings (including juniors):",
        shuffled530.length,
      );
      console.log("Junior pairings (not 5:00pm):", shuffledJuniors.length);
      console.log("Regular pairings:", shuffledRegular.length);

      // 1. Assign 5:00pm preferences to 5:00pm slot FIRST (max 4, randomly selected)
      for (const pairing of shuffled530) {
        if (timeSlotAssignments["5:00pm"].length < 4) {
          timeSlotAssignments["5:00pm"].push({
            ...pairing,
            time_slot: "5:00pm",
          });
        } else {
          // If 5:00 is full, treat juniors as junior pairings, adults as regular
          const hasJunior = pairing.player1_junior || pairing.player2_junior;
          if (hasJunior) {
      ...
      const juniorSlots = ["5:00pm", "5:30pm", "6:00pm", "6:30pm", "7:00pm"];
      ...
      for (const pairing of shuffledJuniors) {
        ...
        const earliestTime = Math.max(
          timeToMinutes(pairing.player1_earliest || "5:00pm"),
          timeToMinutes(pairing.player2_earliest || "5:00pm"),
        );
      ...
      const availableSlots = timeSlots.filter((slot) => slot !== "5:00pm");
    ...
    try {
      ...
      if (player2_id) {
        ...
        if (player2Pos.rows.length === 0) {
          return res
            .status(404)
            .json({ error: "Player 2 not found on ladder" });
        }
